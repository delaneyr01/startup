...
const MonthView = () => {
    ...
    const loadEvents = async () => {
        ...
        try {
            ...
            const response = await fetch(`http://localhost:4000/getEventsForUser?username=${currentUsername}`);
            if (!response.ok) {
    ...
    const addEventToDatabase = async (eventName, eventDescription, eventDate) => {
        ...
        const response = await fetch('http://localhost:4000/addEventToDatabase', {
            method: 'POST',
            headers: {
    ...
    const loadEventsFromLocalStorage = async () => {
        ...
        if (storedUsername) {
            const response = await fetch(`http://localhost:4000/getEventsForUser?username=${storedUsername}`);
            if (response.ok) {
                const events = await response.json();
                updateCalendarWithEvents(events);
            } else {
